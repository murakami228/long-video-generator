...
### 5. テンプレート運用ルール
- **デザインの統一**
  - 全てのテンプレートは、フォント、配色、余白、ヘッダーデザインなどの基本的な「トンマナ（Tone & Manner）」を統一すること。
  - バラバラな世界観（サイバー、和風、ポップなど）を無秩序に混在させない。
  
- **テンプレートの追加**
  - 台本の内容に最適化されたデザインが既存の `slide_templates/` に存在しない場合は、無理に既存のものを使わず、**新しいテンプレートを作成して追加すること**。
  - 新規作成時も、既存の統一デザインルール（パディング、フォント等）を遵守する。

- **アイコンの利用**
  - **絵文字（Emoji）の使用は禁止する**。
  - 代わりに **[Material Symbols and Icons](https://fonts.google.com/icons)** を使用すること。
  - CSS内でGoogle Fontsから読み込み、`<span class="material-symbols-outlined">icon_name</span>` の形式で実装する。
  - アイコンの表示を安定させるために、以下のCSSクラスを必ず定義に含めること：
    ```css
...
### 6. スライド内テキストの制限
- **1行35文字以内**: 字幕（テロップ）同様、スライド内のテキストも1行35文字以内を推奨する。
- 視認性を高めるため、文字を詰めすぎず、要点を絞って記述すること。
